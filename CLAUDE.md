...
- En escritorio el marco NO usa proporción: mide lo que la tira a lo ancho y lo que quepa en la pantalla a lo alto, y el video se contiene con franjas crema. En ≤1000 el marco toma la proporción del primer video (`--pd-frame-ar` la escribe Liquid en el style de `.pdpage`; el JS no la toca), así fotos y video miden lo mismo, con tope `78 * var(--svh)` de alto; la tira de miniaturas se desplaza en horizontal.
