...
| PDP | `product.json`, `product.juguete.json`, `product.caja.json`, `product.suscripcion.json` | `producto-bungot` (split fijo 50/50: galería a la izquierda sobre el color de la familia —desde 2026-09-07 la galería **cabe en la pantalla**: `.pdgal` mide `--pd-gal-h` (100 svh − header − 2×`--pd-gal-pad`, mínimo 480), el marco estira lo que sobra y la tira de miniaturas va **debajo con el MISMO ancho** que él (`--pd-frame-w`, 580/500; las miniaturas se reparten ese ancho, cuadradas, de 56 a 140); solo la foto principal (la bolsa) va a sangre, las demás fotos —infografías, perros— se **contienen** completas sobre crema (`data-fit="contain"`; a sangre se cortaban) y el video también se contiene; sin flechas ni contador, se cambia de foto tocando la miniatura o con el teclado. En ≤1000 el marco va al 100% de ancho con **una sola proporción para todos los productos, 9:16** (`--pd-frame-ar` fija en `producto.css`; desde 2026-09-07 ya no la escribe Liquid con el primer video, porque los productos sin video caían a cuadrado y medían distinto) y un tope de alto para que marco + tira quepan en `--svh` (medido con la barra de Chrome iOS puesta)— y la compra a la derecha **sobre crema a todo lo ancho, sin tarjeta** (se probó el split entero de color con la compra en tarjeta y se regresó a pedido ese mismo día: al usuario le gusta el panel de compra crema); cinturón, cruzada; la plantilla de las cajas prearmadas trae acordeones/beneficios de caja con stepper y cruzada prendidos; la de suscripción —asignada a la Caja BUNGOT, hoy en borrador— apaga el stepper de cantidad y la venta cruzada con los ajustes `show_stepper` / `show_cross` y trae acordeones y cinturón propios de la suscripción, nada de ingredientes/cuidados/envíos) | producto.css/js | `.pdpage` |
