...
### Cajas prearmadas (compra única) — lo que se vende HOY
Desde 2026-09-03 las cajas son **paquetes prehechos de compra única**, no suscripción: productos normales de tipo `Caja` en la colección `cajas-bungot`, con plantilla propia `product.caja.json` (`templateSuffix: caja` en el admin — mismo `producto-bungot`, con stepper y venta cruzada prendidos y acordeones/beneficios de caja). Tienen su **familia de catálogo `cajas`** (`fam_cajas_*`, rosa PANTONE 709 C `#EB5F7A`): banda propia hasta arriba del catálogo, chip de filtro "Cajas" (solo sale si hay cajas) y color consistente en PDP/cruzada/búsqueda vía `card-color.liquid`. Los paneles de portada (`feature-panel`) y catálogo (bloque `suscripcion` de `collection-bandas` — el tipo conserva el nombre viejo a propósito) anuncian las cajas: su CTA con link vacío cae a la banda `#banda-cajas` (ancla que exponen todas las bandas, con `scroll-margin-top` en `productos.css`); el del catálogo ancla en la misma página y el de la portada va a `fam_cajas_collection.url + '#banda-cajas'`. El contenido de cada caja va en su **descripción** (los 3 productos de 2026-09-03 —Bienvenida, Cumpleañera, Michi— son placeholders con la foto de la caja de la suscripción; el cliente los va a rearmar). No hay picker ni metafields para las cajas prearmadas: son SKUs fijos.
