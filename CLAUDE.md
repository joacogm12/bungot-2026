...
| PDP | `product.json`, `product.juguete.json`, `product.caja.json`, `product.suscripcion.json` | `producto-bungot` (split fijo 50/50: galería a la izquierda sobre el color de la familia —desde 2026-09-07 marco de **proporción única** para fotos y video: `--pd-frame-ar` la pone Liquid en `.pdpage` con el primer video del producto (sin video cae a 1, cuadrado), las fotos van a sangre recortadas a esa proporción y el marco nunca brinca al cambiar de pieza; cabe en `--pd-frame-max-w × --pd-frame-max-h`; tira de miniaturas **debajo**, sin flechas ni contador, se cambia de foto tocando la miniatura o con el teclado— y la compra a la derecha **sobre crema a todo lo ancho, sin tarjeta** (se probó el split entero de color con la compra en tarjeta y se regresó a pedido ese mismo día: al usuario le gusta el panel de compra crema); cinturón, cruzada; la plantilla de las cajas prearmadas trae acordeones/beneficios de caja con stepper y cruzada prendidos; la de suscripción —asignada a la Caja BUNGOT, hoy en borrador— apaga el stepper de cantidad y la venta cruzada con los ajustes `show_stepper` / `show_cross` y trae acordeones y cinturón propios de la suscripción, nada de ingredientes/cuidados/envíos) | producto.css/js | `.pdpage` |
...
- El marco tiene la proporción del primer video en TODOS los anchos (`--pd-frame-ar` la escribe Liquid en el style de `.pdpage`; el JS ya no la toca), así fotos y video miden lo mismo; en móvil el tope es `78 * var(--svh)` de alto y la tira de miniaturas se desplaza en horizontal.
